    ...
    <div class="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl border border-pastel-peach/30 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="text-center mb-8">
                ...
                <h2 class="text-3xl font-bold bg-gradient-to-r from-orange-400 to-pink-500 bg-clip-text text-transparent">Daftar Akun</h2>
                <p class="text-sm text-gray-400 font-semibold mt-2">Buat akun untuk memulai produktivitasmu</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input id="name" class="block w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-pastel-peach focus:ring focus:ring-pastel-peach/20 outline-none transition-all duration-200 bg-gray-50/50 text-gray-700" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" />
                    @error('name')
                ...
                <div>
                    ...
                    <input id="email" class="block w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-pastel-peach focus:ring focus:ring-pastel-peach/20 outline-none transition-all duration-200 bg-gray-50/50 text-gray-700" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" />
                    @error('email')
                        <span class="text-xs text-red-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="mt-6 text-center text-sm text-gray-500 font-semibold">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="text-pink-500 hover:text-pink-600 hover:underline transition">Masuk di sini</a>
            </div>
        </div>
    </div>
