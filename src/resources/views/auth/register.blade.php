...
<head>
    ...
    <title>PomoTasky - Buat Akun Baru</title>
        ...
        <div class="relative z-10">
            ...
            <div class="text-center mb-8">
                <a href="{{ url('/') }}" class="inline-block hover:opacity-80 transition-opacity">
                    ...
                    <h2 class="text-4xl font-extrabold bg-gradient-to-r from-[#F46B7E] to-[#10B981] bg-clip-text text-transparent">Daftar Akun</h2>
                </a>
                <p class="text-sm text-gray-500 font-semibold mt-2">Buat akun untuk memulai produktivitasmu</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input id="name" class="block w-full px-4 py-3 rounded-xl border border-white focus:border-emerald-500 outline-none transition-all duration-200 bg-white/70 text-gray-700 shadow-sm input-pastel" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" />
                    @error('name')
                ...
                <div>
                    ...
                    <input id="email" class="block w-full px-4 py-3 rounded-xl border border-white focus:border-emerald-500 outline-none transition-all duration-200 bg-white/70 text-gray-700 shadow-sm input-pastel" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" />
                    @error('email')
                        <span class="text-xs text-red-500 font-semibold mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mt-4">
                    <a href="{{ route('auth.google', ['intent' => 'register']) }}" class="w-full flex items-center justify-center gap-2 bg-white/80 hover:bg-white border border-white text-gray-700 font-bold py-3 px-6 rounded-xl shadow-sm transition-all duration-200 ease-in-out">
            ...
            <div class="mt-6 text-center text-sm text-gray-600 font-semibold">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="text-pink-accent hover:underline transition">Masuk di sini</a>
            </div>
        </div>
    </div>
